    ...
    class Monitor extends EventTarget {
        ...
        #dataUsage() {
            ...
            // If no-cors is enabled, we cannot reliably measure payload sizes
            if (this.#noCors) {
                return {
                    usage: null,
                    stability: 0,
            ...
            const payloadVariationRatio =
                payloadStandardDeviation / (averagePayloadBytes + EPSILON);

            // Stability score: 1 = very consistent, 0 = very unstable
            const payloadStabilityScore = Math.max(
                0,
                Math.min(1, 1 - payloadVariationRatio)
            );
            ...
            const estimateRequests = (intervalMs) => {
                ...
                const bytesPerMinute = averagePayloadBytes * requestsPerMinute;
                const bytesPerHour = averagePayloadBytes * requestsPerHour;
                const bytesPerDay = averagePayloadBytes * requestsPerDay;
            ...
            return {
                ...
                projections: Object.fromEntries(
                    Object.entries(this.#pollingIntervals).map(
                        ([intervalName, intervalMs]) => [
                            intervalName,
                            estimateRequests(intervalMs)
                        ]
                    )
                )
            };
